...
TEST(Collatz, eval_8) {
	const int v = collatz_eval(986075, 986074);
	ASSERT_EQ(215, v);
}
...
TEST(Collatz, cycle_length3) {
	const int c = cycle_length(123456);
	ASSERT_EQ(62, c);
}

TEST(Collatz, cycle_length4) {
	const int c = cycle_length(986689);
	ASSERT_EQ(122, c);
}

TEST(Collatz, cycle_length5) {
	const int c = cycle_length(999999);
	ASSERT_EQ(259, c);
}
...
TEST(Collatz, print_3) {
	std::ostringstream w;
	collatz_print(w, 986075, 809598, 525);
	ASSERT_EQ("986075 809598 525\n", w.str());
}
