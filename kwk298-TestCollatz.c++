...
//Only one integer value (no space after)
TEST(Collatz, read_4) {
...
//Only one integer value (space after)
TEST(Collatz, read_5) {
    std::istringstream r("10 ");
    const std::pair<int, int> p = collatz_read(r);
    ASSERT_EQ(10, p.first);
...
//Only one integer value (newline after)
TEST(Collatz, read_6) {
    std::istringstream r("10 \n");
    const std::pair<int, int> p = collatz_read(r);
    ASSERT_EQ(10, p.first);
...
//Printing floats should truncate to ints
TEST(Collatz, print_2) {
    std::ostringstream w;
    collatz_print(w, 1.1, 10.1, 20.1);
    ASSERT_EQ("1 10 20\n", w.str());}

//Printing chars prints the ascii number
